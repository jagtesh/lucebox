...
// === ROCMFP vec_dot device functions (ported from ROCmFPX) ===
...
static __device__ __forceinline__ int rocmfpx_decode_fp2_code_vec_cuda(const uint32_t code) {
    switch (code & 3u) {
...
static __device__ __forceinline__ int rocmfpx_pack4_fp3_vec_cuda(const uint8_t * qs, const int base) {
    ...
    const int bit_shift = bit_pos & 7;
    uint32_t packed = (uint32_t) qs[byte_pos] |
                      ((uint32_t) qs[byte_pos + 1] << 8);
    // A 12-bit group beginning at bit offsets 5..7 reaches a third byte.
    // Avoid that load for offsets 0..4 so the final in-bounds group does not
    // read past a tightly packed quant block.
    if (bit_shift >= 5) {
        packed |= (uint32_t) qs[byte_pos + 2] << 16;
    }
    return rocmfpx_pack4_fp3_bits12_vec_cuda(
        (packed >> bit_shift) & 0x0fffu);
}
...
static __device__ __forceinline__ int rocmfpx_pack4_fp2_vec_cuda(const uint8_t * qs, const int base) {
    return rocmfpx_pack4_fp2_bits8_vec_cuda(qs[base >> 2]);
}
...
static __device__ __forceinline__ float vec_dot_rocmfpx_fp2_q8_1(
    ...
    // iqs in {0,1} selects one 16-weight half-block: qs bytes [4*iqs .. 4*iqs+3] + scale e[iqs].
    // qs is a 10-byte-strided, byte-aligned array -> read byte-wise (uint8_t index); NEVER cast qs to int*/int2*.
...
static __device__ __forceinline__ float vec_dot_rocmfpx_fp6_q8_1(
    ...
    // Trailing 0 pad: the last FP6 window reads qs[reg_idx + 1] with reg_idx==5;
    // its high bits are always masked out, so 0 keeps the result bit-identical
    // while avoiding a stack over-read (matches the FP3 sibling below).
